...
have_node_container() { docker ps --format '{{.Names}}' 2>/dev/null | grep -qx remnanode; }
...
validate_config() {
  ...
  docker exec -i remnanode sh -c 'cat > /tmp/rnk-test.json' <<<"$cfg" 2>/dev/null || {
    warn "Не смог положить файл в контейнер, пропускаю валидацию"; return 0; }

  local out rc=0
  out=$(docker exec remnanode xray run -test -c /tmp/rnk-test.json 2>&1) || rc=$?
  if (( rc != 0 )); then
    out=$(docker exec remnanode xray -test -config /tmp/rnk-test.json 2>&1) && rc=0 || rc=$?
  fi
  docker exec remnanode rm -f /tmp/rnk-test.json 2>/dev/null || true
...
do_xhttp_api() {
  ...
  api PATCH /config-profiles "$(jq -n --arg u "$pu" --argjson c "$newcfg" '{uuid:$u, config:$c}')" >/dev/null \
    && ok "Профиль обновлён" || { err "PATCH не прошёл. Откат: $0 --rollback"; return 1; }
  ...
  # ---- uuid инбаунда
  local iu
  iu=$(api GET /config-profiles/inbounds \
       | jq -r --arg t "$INBOUND_TAG" '(.response.inbounds // .response) | map(select(.tag==$t)) | .[0].uuid // empty')
  [[ -z "$iu" ]] && { warn "uuid инбаунда не нашёл — Host заведи руками"; do_postflight; return 0; }
  ok "Inbound uuid: $iu"
  ...
  api POST /hosts "$host_body" >/dev/null 2>&1 && ok "Host создан" || {
    warn "POST /hosts не прошёл — схема панели могла поменяться. Тело запроса:"; jq . <<<"$host_body"; }

  [[ "$API_SCOPE" == "host" ]] && { do_postflight; return 0; }

  # ---- нода
  local nodes nu node cur node_body
  nodes=$(api GET /nodes | jq '.response.nodes // .response')
  nu=$(pick "$nodes" name uuid "Нода" "$NODE_NAME") || { do_postflight; return 0; }
  ...
  api PATCH /nodes "$node_body" >/dev/null 2>&1 && ok "Инбаунд включён на ноде" || {
    warn "PATCH /nodes не прошёл. Тело:"; jq . <<<"$node_body"; }

  # ---- сквад (без него инбаунд не попадёт в подписки)
  local squads su squad sinb squad_body
  squads=$(api GET /internal-squads | jq '.response.internalSquads // .response')
  su=$(pick "$squads" name uuid "Internal Squad" "$SQUAD_NAME") || { do_postflight; return 0; }
  ...
  api PATCH /internal-squads "$squad_body" >/dev/null 2>&1 && ok "Добавлен в сквад" || {
    warn "PATCH /internal-squads не прошёл. Тело:"; jq . <<<"$squad_body"; }
...
do_postflight() {
  ...
  if have_node_container; then
    sleep 3
    local xl
    xl=$(docker exec remnanode xlogs 2>&1 | tail -40)
    if grep -qiE 'SPAWN_ERROR|failed to (parse|start)|panic' <<<"$xl"; then
      ...
      dim "Подробности: docker exec -it remnanode tail -n 50 /var/log/supervisor/xray.err.log"
    else
